...
public class Script: NSObject, NSCopying {
    ...
    private let onMessage: MessageHandler = { _, rawMessage, rawData, userData in
        ...
        if let dict = message as? [String: Any],
            let type = dict["type"] as? String, type == "send",
            let payload = dict["payload"] as? [Any], let kind = payload.first as? String,
            kind == "frida:rpc",
            ...
            if success == "ok" {
                ...
                if payload.count >= 3, let string = payload[2] as? String {
                    ...
                    message = "Failed to cast error payload as string: \(payload)"
                }
